...
def _main(args, frames_paths, crops_bboxes, crop_value, resize_frames=None, verbose=1, person_only=True, allowed_number_of_boxes=100):
    ...
    for frame_i in range(0,len(frames_paths)):
        ...
        for crop_i in range(0, len(crops_in_frame)):
            ...
            import cv2
            path = '/home/dl/lyx/myapl/cropdebug/unprocess/testcrop-frame{}-crop{}.jpg'.format(frame_i,crop_i)
            ...
            # cv2.imshow('window_title', imaget)
            ...
            for i, c in reversed(list(enumerate(out_classes))):
                ...
                # print(box)
                # print(type(box))
                # first = (int(x-h/2),int(y-w/2))
                # second = (int(x+h/2),int(y+w/2))
                # print(box)
                draw_0 = cv2.rectangle(imaget,(box[1],box[0]),(box[3],box[2]),(255, 0, 0),1)
                # cv2.imshow('test', draw_0)
            if draw_0 is not None:
                cv2.imwrite("/home/dl/lyx/myapl/cropdebug/result/testcrop-frame{}-crop{}.jpg".format(frame_i,crop_i), draw_0)
                # cv2.imshow("frame{}-crop{}.jpg".format(frame_i,crop_i),draw_0)
                # cv2.waitKey(0)
