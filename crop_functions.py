...
def crop_from_one_img(img, horizontal_splits, overlap_px, scale, show=False, save_crops=True, frame_name=''):
...
def crop_from_one_frame(frame_path, horizontal_splits,overlap_px, show, save_crops=True, save_visualization=True):
    # crop*scale is the size inside input image
    # crop is the size of output image

    frame_name = os.path.basename(frame_path)
    frame_name = frame_name[0:-4]
    print('test'+frame_name)
    # if save_crops:
...
#@profile
def crop_from_one_frame_WITH_MASK_in_mem(img, mask, frame_path, horizontal_splits, overlap_px, mask_over, show, save_crops=True, save_visualization=True):
    ...
    # 通过检查mask对应区域中是否画过，来筛选有可疑对象的crops
    for w_crop in w_crops:
        for h_crop in h_crops:
            area = (int(w_crop[0]), int(h_crop[0]), int(w_crop[0] + crop), int(h_crop[0] + crop))
            cropped_img = img.crop(box=area)
...
#@profile
def mask_from_one_frame(frame_path, SETTINGS):
    ...
    mask_crops, crop = crop_from_one_img(tmp, horizontal_splits, overlap_px, 1.0, frame_name=frame_name+"/", save_crops=save_crops)
    # print("123")
